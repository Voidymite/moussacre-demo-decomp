...
if (place_meeting(x, y, obj_object_saw_horizontal) ? true : (place_meeting(x, y, obj_object_saw_vertical) ? true : (place_meeting(x, y, obj_hitbox_sawdrone_attack_1) ? true : place_meeting(x, y, obj_hitbox_sawdrone_attack_2))))
{
    ...
    instance_create_depth(x, y, -10, obj_particle_enemy_dead_3).sprite_index = spr_enemy_hedgesoldier_dead_1
    instance_create_depth(x, y, -10, obj_particle_enemy_dead_4).sprite_index = spr_enemy_hedgesoldier_dead_2
    repeat (36)
...
if (place_meeting(x, y, obj_hitbox_spin) ? true : (place_meeting(x, y, obj_hitbox_doublejump) ? true : (place_meeting(x, y, obj_hitbox_groundpound) ? true : (place_meeting(x, y, obj_object_waterspout) ? true : (place_meeting(x, y, obj_object_collapse) ? true : (place_meeting(x, y, obj_hitbox_shoot) ? true : (place_meeting(x, y, obj_hitbox_hook_3) ? true : (place_meeting(x, y, obj_hitbox_bomb_2) ? true : crash))))))))
{
    if place_meeting(x, y, obj_hitbox_spin)
    {
        ...
        instance_create_depth(x, y, -10, obj_particle_enemy_dead_1).sprite_index = spr_enemy_hedgesoldier_dead_3
        instance_create_depth(x, y, -10, obj_particle_enemy_dead_2).sprite_index = spr_enemy_hedgesoldier_dead_4
        if drop
        {
            repeat (5)
    ...
    if (place_meeting(x, y, obj_hitbox_shoot) ? true : (place_meeting(x, y, obj_hitbox_hook_3) ? true : place_meeting(x, y, obj_hitbox_bomb_2)))
    {
        ...
        instance_create_depth(x, y, -10, obj_particle_enemy_dead_5).sprite_index = spr_enemy_hedgesoldier_dead_5
...
if (scared_time > 2)
{
    sprite_index = spr_enemy_hedgesoldier_scared
    if (image_index == 4)
        image_index = 2
}
if (scared_time == 2)
...
if (sprite_index == spr_enemy_hedgesoldier_attack)
{
    if (place_meeting(x, y, obj_mick) && (!(place_meeting(x, y, obj_hitbox_spin))) && (!(place_meeting(x, y, obj_hitbox_doublejump))) && (!(place_meeting(x, y, obj_hitbox_groundpound))))
    {
        with (obj_mick)
...
if attacking
{
    charge -= 1
    if (charge == 44)
        audio_play_sound(snd_enemy_hedgesoldier_attack_1, 1, 0)
    if (charge == 1)
        audio_play_sound(snd_enemy_hedgesoldier_attack_2, 1, 0)
    if (charge > 0)
        sprite_index = spr_enemy_hedgesoldier_charge
    else
        sprite_index = spr_enemy_hedgesoldier_attack
    if (sprite_index == spr_enemy_hedgesoldier_charge)
    {
        if (image_index > 5)
            image_index = 3
    }
    if (charge <= 0)
        hspd = 100 * (-image_xscale)
}
if (hspd > 0)
    hspd -= 1
if (hspd < 0)
...
if place_meeting((x + hspd), y, obj_wall)
{
    while (!(place_meeting((x + sign(hspd)), y, obj_wall)))
        x += sign(hspd)
    hspd = 0
    if (sprite_index != spr_enemy_hedgesoldier_scared)
        crash = 1
}
