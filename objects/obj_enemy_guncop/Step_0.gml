...
if (place_meeting(x, y, obj_hitbox_spin) || place_meeting(x, y, obj_hitbox_doublejump) || place_meeting(x, y, obj_hitbox_groundpound) || place_meeting(x, y, obj_object_waterspout) || place_meeting(x, y, obj_object_collapse) || place_meeting(x, y, obj_hitbox_shoot) || place_meeting(x, y, obj_hitbox_hook_3) || place_meeting(x, y, obj_hitbox_bomb_2) || place_meeting(x, y, obj_hitbox_deflect) || place_meeting(x, y, obj_hitbox_counter) || place_meeting(x, y, obj_hitbox_rang))
{
    if (place_meeting(x, y, obj_hitbox_spin) || place_meeting(x, y, obj_hitbox_rang))
    {
        ...
        instance_create_depth(x, y, -10, obj_particle_enemy_dead_1).sprite_index = spr_enemy_guncop_dead_1;
        instance_create_depth(x, y, -10, obj_particle_enemy_dead_2).sprite_index = spr_enemy_guncop_dead_2;
...
if (attacking)
{
    ...
    if (image_index == 8)
    {
        audio_play_sound(snd_enemy_guncop_attack_1, 1, 0);
        audio_play_sound(snd_enemy_guncop_attack_voice_1, 1, 0);
    }
    
    if (image_index == 16)
    {
        audio_play_sound(snd_enemy_guncop_attack_2, 1, 0);
        
        if (image_xscale == 1)
        {
            instance_create_depth(x - 100, y - 64, -4, obj_hitbox_guncop_attack);
            instance_create_depth(x - 100, y - 64, 10, obj_particle_gunshot);
        }
        else
        {
            instance_create_depth(x + 100, y - 64, -4, obj_hitbox_guncop_attack).image_xscale = -1;
            instance_create_depth(x + 100, y - 64, 10, obj_particle_gunshot);
        }
    }
}
